...
from src.synthetic_ner.metadata.version import get_version_provenance
...
def _report_header(
    *,
    ...
    return [
        ...
        _format_revision_policy(context.workflow_cfg),
        f"- Memory file: {memory_path}",
        f"- MLflow enabled: {str(trace_info.enabled).lower()}",
        f"- MLflow trace id: {trace_info.trace_id or 'n/a'}",
        f"- MLflow trace url: {trace_info.trace_url or 'n/a'}",
    ...
    for section_name in section_contracts:
        ...
        lines.extend(
            [
                f"### {section_name}",
                "",
                f"Status: {'unresolved' if issues else 'approved'}",
                "",
                "Contract:",
...
def _format_node_analytics(node_summary: list[dict[str, Any]]) -> list[str]:
    lines = [
        ...
        "",
        "Each node below also appears as its own child span in MLflow.",
        "",
...
def _format_revision_policy(workflow_cfg) -> str:
    if not workflow_cfg.polisher.active or workflow_cfg.max_revisions == 0:
        return "- Revision policy: no polisher revisions"
    return (
        "- Revision policy: writer runs once; critic and validators review each draft; "
        f"up to {workflow_cfg.max_revisions} polisher revision(s)"
    )
