...
from src.synthetic_ner.tasks.document_generation.constants import RUBRIC_LINE_RE
...
class SectionCritic:
    ...
    def _parse_result(self, raw_text: str) -> CriticResult:
        try:
            ...
            return _parse_legacy_result(raw_text, self.acceptance_threshold)
        ...
        critical_rubric_issues = _critical_rubric_issues(rubrics)
        for issue in critical_rubric_issues:
            if issue not in issues:
                issues.append(issue)
        if critical_rubric_issues:
            blocking = True
...
def _parse_legacy_result(raw_text: str, acceptance_threshold: float = 3.5) -> CriticResult:
...
def _parse_rubrics(rubric_block: Any) -> dict[str, int]:
    ...
    rubrics: dict[str, int] = {}
    for metric, raw_score in RUBRIC_LINE_RE.findall(rubric_block):
...
def _critical_rubric_issues(rubrics: dict[str, int]) -> list[str]:
    return [
        f"Critic rubric '{metric}' is blocking with score {score}/5."
        for metric, score in rubrics.items()
        if metric in {"grounding", "completeness"} and score <= 2
    ]


def _overall_rubric_score(rubrics: dict[str, int]) -> float:
    if not rubrics:
        return 0.0
...
def _blocking_rubric_issues(rubrics: dict[str, int]) -> list[str]:
    return _critical_rubric_issues(rubrics)
